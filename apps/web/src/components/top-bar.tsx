...
export function TopBar({ displayName, handle, avatarSeed }: Props) {
  ...
  return (
    <header className="hidden lg:flex fixed top-4 right-4 h-[var(--topbar-height)] glass-topbar rounded-2xl border border-white/10 z-40 items-center justify-between px-6 left-[calc(var(--sidebar-width)+1rem)]">
      <div className="flex items-center gap-8">
        ...
        <div className="flex items-center gap-3 px-6 border-l border-white/10">
          <div className="text-right">
            <p className="text-[10px] text-neutral-500 uppercase tracking-widest font-bold">Precisão</p>
            <p className="text-xs text-green-500 font-mono font-bold">98.2%</p>
          </div>
          <div className="w-10 h-10 rounded-full border-2 border-green-500/30 flex items-center justify-center relative">
            <div className="absolute inset-1 rounded-full border border-green-500/20 animate-ping" />
            <span className="text-[10px] font-black text-green-500">MAX</span>
          </div>
        </div>
      </div>
