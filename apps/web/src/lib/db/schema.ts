...
import {
  pgTable, uuid, text, integer, boolean, timestamp, jsonb, index, vector, numeric,
...
export const profiles = pgTable('profiles', {
  ...
  genre: text('genre'),
  stage: text('stage').default('pendente'),      // pendente | iniciante | intermediario | avancado
  confrontoLevel: integer('confronto_level').default(1), // 1-5
  ...
  faseCarreira: text('fase_carreira'),
  nivelTecnico: text('nivel_tecnico'),
  arquetipoEstrategico: jsonb('arquetipo_estrategico'),
  network: integer('network').default(0),
...
export const contacts = pgTable(
  'contacts',
  {
    ...
    email: text('email'),
    phone: text('phone'),
    category: text('category'),         // artist | agent | producer | press
    status: text('status').notNull().default('lead'), // lead | negotiation | closed
    notes: text('notes'),
...
export const kanbanCards = pgTable('kanban_cards', {
  ...
  source: text('source'),               // Manual, Estratégia, Gerador, etc.
  tag: text('tag'),                     // Marketing, Design, A&R, etc.
...
export const kanbanCardLogs = pgTable('kanban_card_logs', {
  id: uuid('id').primaryKey().default(sql`gen_random_uuid()`),
  cardId: uuid('card_id').notNull(),
  userId: uuid('user_id').notNull(),
  action: text('action').notNull(),     // create | move | edit | delete
  details: jsonb('details'),            // old/new values
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow(),
})
...
// ----------------------------------------------------------------
// classifications — Módulo Classificador AI
// ----------------------------------------------------------------

export const classifications = pgTable('classifications', {
  id: uuid('id').primaryKey().default(sql`gen_random_uuid()`),
  userId: uuid('user_id').notNull(),
  source: text('source').notNull(), // URL or File Name
  type: text('type').notNull(), // 'link' | 'file'
  category: text('category'), // e.g., 'Release', 'Contract', 'Demo'
  status: text('status').notNull().default('pending'), // 'pending' | 'processing' | 'completed' | 'failed'
  result: jsonb('result'), // Classification details (tags, confidence, etc.)
  justification: text('justification'),
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow(),
})

// ----------------------------------------------------------------
// memories — Inteligência de Memória (Memory Engine)
// ----------------------------------------------------------------
...
export const memories = pgTable(
  'memories',
  {
    ...
    embedding: vector('embedding', { dimensions: 384 }), // Local HuggingFace standard (paraphrase-multilingual-MiniLM-L12-v2)
    metadata: jsonb('metadata'), // { source: 'checkin', importance: 5, etc }
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow(),
  },
  (t) => ({
    userIdIdx: index('memories_user_idx').on(t.userId),
  })
)
