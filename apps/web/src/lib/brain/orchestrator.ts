...
export async function runOnboarding(
  ...
  const [analysis, classification, strategy] = await Promise.all([
    ...
    runClassifier(cleaned).catch(() => ({
      ...
      fase_carreira: 'residente_local' as const,
      nivel_tecnico: 'intermediario' as const,
      arquetipo_estrategico: ['produtor_estudio'] as string[],
      network: 1 as const,
    })),
...
const effectiveStrategy = strategy ?? {
    ...
    posicionamento: { nicho: cleaned.referencias, publica: 'A definir', cena: cleaned.referencias, concorrentes_referencia: [] },
    ...
    acoes_obrigatorias: ['Definir objetivos', 'Criar primeiro release', 'Mapear mercado'],
    acao_proibida: 'Aceitar gigs gratuitos sem estratégia',
    objetivo_unico: 'Definir direção estratégica',
    eixos_ataque: ['produção', 'networking'],
    acoes_semanais: [
      { dia: 'terca', tarefa: 'Estudar mercado', tempo: '2h', entregue: 'notas' },
    ],
  }
  ...
  return {
    ...
    brain: {
      nivel: brain.nivel,
      confronto: brain.confronto,
      resumo: brain.analise.problema_central,
      prioridade: brain.direcao.frase_norte,
